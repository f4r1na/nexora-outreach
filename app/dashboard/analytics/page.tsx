  ...
  return (
    <>
      ...
      <header style={{
        ...
        <div>
          <h1 style={{ fontSize: 15, fontWeight: 500, color: "#fff", fontFamily: "var(--font-syne)", lineHeight: 1 }}>
            Analytics
          </h1>
          <p style={{ fontSize: 12, color: "#555", fontFamily: "var(--font-outfit)", marginTop: 2 }}>
      ...
      <main style={{ flex: 1, padding: "24px 32px 64px" }}>
        {planLoading ? (
          ...
          <div style={{
            ...
            <div>
              <h2 style={{ fontSize: 16, fontWeight: 500, color: "#fff", fontFamily: "var(--font-syne)", marginBottom: 6 }}>
                Analytics requires Pro
              </h2>
              <p style={{ fontSize: 13, color: "#555", fontFamily: "var(--font-outfit)", lineHeight: 1.6, maxWidth: 340 }}>
          ...
          <div style={{ color: "#444", fontFamily: "var(--font-outfit)", fontSize: 13 }}>Loading analytics…</div>
          ...
          <div style={{
            ...
            <Link href="/dashboard/campaigns/new" style={{
              fontSize: 13, color: "#FF5200", fontFamily: "var(--font-outfit)", textDecoration: "none",
            }}>
              Create campaign
            </Link>
          </div>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 24 }}>
              {[
                { label: "EMAILS SENT",  value: stats?.sent ?? 0 },
                { label: "OPEN RATE",    value: `${stats?.open_rate ?? 0}%`,   sub: `${stats?.opened ?? 0} opens` },
                { label: "CLICK RATE",   value: `${stats?.click_rate ?? 0}%`,  sub: `${stats?.clicked ?? 0} clicks` },
                { label: "REPLY RATE",   value: `${stats?.reply_rate ?? 0}%`,  sub: `${stats?.replied ?? 0} replies` },
              ].map((card) => (
                <div key={card.label} style={{
                  ...
                  <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: "0.06em", color: "#555", fontFamily: "var(--font-outfit)", marginBottom: 8 }}>
                    {card.label}
                  </div>
                  ...
                  {card.sub && (
                    <div style={{ fontSize: 11, color: "#555", fontFamily: "var(--font-outfit)" }}>
                      {card.sub}
                    </div>
                  )}
                </div>
            ...
            <div style={{
              ...
              <p style={{
                ...
                color: "#555", fontFamily: "var(--font-outfit)", marginBottom: 16,
              }}>
              ...
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={daily} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
                  <XAxis
                    dataKey="date"
                    tick={{ fill: "#444", fontSize: 10, fontFamily: "var(--font-outfit)" }}
            ...
            {campaigns.length > 0 && (
              <div style={{
                ...
                {campaigns.map((camp, idx) => (
                  ...
                  >
                    <div>
                      <p style={{ fontSize: 13, color: "#ccc", fontFamily: "var(--font-outfit)", margin: 0 }}>
                        {camp.name}
                      </p>
                      <p style={{ fontSize: 11, color: "#444", fontFamily: "var(--font-outfit)", margin: 0, marginTop: 1 }}>
                        {new Date(camp.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                      </p>
                    </div>
                    <span style={{ fontSize: 13, color: "#888", fontFamily: "var(--font-outfit)" }}>{camp.sent}</span>
                    <span style={{ fontSize: 13, color: "#888", fontFamily: "var(--font-outfit)" }}>{camp.opened}</span>
                    <span style={{ fontSize: 13, color: rateColor(camp.open_rate), fontFamily: "var(--font-outfit)" }}>{camp.open_rate}%</span>
                    <span style={{ fontSize: 13, color: "#888", fontFamily: "var(--font-outfit)" }}>{camp.clicked}</span>
                    <span style={{ fontSize: 13, color: rateColor(camp.click_rate), fontFamily: "var(--font-outfit)" }}>{camp.click_rate}%</span>
                    <span style={{ fontSize: 13, color: "#888", fontFamily: "var(--font-outfit)" }}>{camp.replied}</span>
